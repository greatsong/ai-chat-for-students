...
// 관리자 전용 메뉴
const adminNavItems = [
  { to: '/teacher/students', label: '사용자 관리', icon: 'users' },
  { to: '/teacher/teachers', label: '교사 관리', icon: 'teacher' },
  { to: '/teacher/conversations', label: '채팅 열람', icon: 'chat' },
  { to: '/teacher/usage', label: '전체 사용량', icon: 'chart' },
  { to: '/teacher/my-usage', label: '내 사용량', icon: 'trending' },
  { to: '/teacher/settings', label: '설정', icon: 'settings' },
];

// 교사 메뉴 (자기 사용량만)
const teacherNavItems = [{ to: '/teacher/my-usage', label: '내 사용량', icon: 'trending' }];
      ...
      >
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white">
              {isAdmin ? icons.shield : icons.trending}
            </div>
            <div>
              <h2 className="text-sm font-bold text-gray-800">{dashboardTitle}</h2>
        ...
        <nav className="flex-1 p-3 flex flex-col gap-0.5">
          {navItems.map((item) => (
            ...
            >
              <span className="text-gray-400">{icons[item.icon]}</span>
              {item.label}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="p-3 border-t border-gray-100">
          <Link
            to="/chat"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:bg-gray-50 hover:text-indigo-600 transition-all duration-150"
          >
            {icons.back}
            채팅으로 돌아가기
          </Link>
