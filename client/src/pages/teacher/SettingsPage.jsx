...
const PROVIDERS = [
  {
    ...
    defaultModels: ['claude-sonnet-4-6', 'claude-sonnet-5', 'claude-opus-4-8'],
    color: 'orange',
  ...
  },
  {
    ...
    company: 'Google',
    placeholder: 'AIzaSy...',
    defaultModels: ['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3.1-pro-preview'],
  ...
  },
  {
    ...
    company: 'OpenAI',
    placeholder: 'sk-proj-...',
    defaultModels: ['gpt-5.5', 'gpt-5.6-luna', 'gpt-5.6-terra', 'gpt-5.6-sol'],
...
export default function SettingsPage() {
  ...
  const loadApiKeyStatus = async () => {
    try {
      setApiKeyStatus(await apiGet('/teacher/api-keys'));
    } catch {
      /* 무시 */
    }
  };
    ...
    <div className="max-w-3xl">
      ...
      <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between p-4 pb-0">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <div className="relative">
                        ...
                        />
                        <div
                          className={`w-10 h-5 rounded-full transition-colors ${isEnabled ? 'bg-blue-600' : 'bg-gray-300'}`}
                        >
                          <div
                            className={`w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform mt-0.5 ${isEnabled ? 'translate-x-5.5 ml-0.5' : 'translate-x-0.5'}`}
                          />
                        </div>
                      </div>
                      <div>
                  ...
                  {isEnabled && (
                    <div className="p-4 pt-3 space-y-3">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          <input
                            ...
                            onChange={(e) =>
                              setNewModelInputs((p) => ({ ...p, [provider.id]: e.target.value }))
                            }
                        ...
                        <div className="border-t border-gray-200/60 pt-3">
                          ...
                          <div className="flex gap-2 mt-2">
                            <input
                              ...
                              onChange={(e) =>
                                setNewImageModelInputs((p) => ({
                                  ...p,
                                  [provider.id]: e.target.value,
                                }))
                              }
                              onKeyDown={(e) =>
                                e.key === 'Enter' && handleAddImageModel(provider.id)
                              }
                      ...
                      <div className="border-t border-gray-200/60 pt-3">
                        ...
                        {!hasKey && (
                          <div className="text-xs text-red-500 mb-1.5">
                            API 키가 설정되지 않았습니다. 이 프로바이더를 사용하려면 키가
                            필요합니다.
                          </div>
                        )}
                        <div className="flex gap-2">
                          <input
                            ...
                            onChange={(e) =>
                              setApiKeyInputs((p) => ({
                                ...p,
                                [provider.apiKeyId]: e.target.value,
                              }))
                            }
        ...
        <section className="bg-white rounded-xl border-2 border-teal-200 p-5">
          ...
          <p className="text-xs text-gray-400 mb-4">
            AI 응답을 음성으로 읽어줍니다. 교사는 항상 사용 가능합니다.
          </p>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">기본 음성</label>
                <select
                ...
                >
                  ...
                  <option value="shimmer">shimmer (따뜻하고 부드러움)</option>
                  <option value="alloy">alloy (중성적)</option>
                  <option value="echo">echo (차분한 남성)</option>
                  <option value="fable">fable (이야기 톤)</option>
                  <option value="onyx">onyx (깊고 무게감)</option>
                </select>
              </div>
              <div>
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
              <label className="flex items-center gap-3 cursor-pointer">
                <div className="relative">
                  ...
                  />
                  <div
                    className={`w-10 h-5 rounded-full transition-colors ${ttsEnabled ? 'bg-teal-600' : 'bg-gray-300'}`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform mt-0.5 ${ttsEnabled ? 'translate-x-5.5 ml-0.5' : 'translate-x-0.5'}`}
                    />
                  </div>
                </div>
                <span className="text-sm text-gray-600">학생에게도 TTS 허용</span>
              </label>
            </div>
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p className="text-xs text-gray-400 mb-4">
            학생들의 모든 AI 대화에 적용될 시스템 프롬프트입니다. 비어있으면 기본 프롬프트가
            사용됩니다.
          </p>
          <textarea
            value={systemPrompt}
          ...
          <button
            ...
            className={`px-6 py-2.5 text-sm font-medium text-white rounded-lg transition-all duration-200 ${
              saving
                ...
                : 'bg-blue-600 hover:bg-blue-700 shadow-sm hover:shadow active:scale-[0.97]'
            }`}
          >
            {saving ? '저장 중...' : '설정 저장'}
          </button>
