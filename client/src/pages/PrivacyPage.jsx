...
const sections = [
  {
    ...
    content: [
      {
        type: 'text',
        value: `당곡고 학생을 위한 AI 채팅(이하 '서비스')은 다음의 목적을 위해 개인정보를 수집·이용합니다.`,
      },
      {
        type: 'list',
        items: [
  ...
  },
  {
    ...
    content: [
      { type: 'subtitle', value: 'Google OAuth 로그인 시 자동 수집' },
      {
        type: 'list',
        items: [
      ...
      { type: 'subtitle', value: '서비스 이용 중 자동 생성' },
      {
        type: 'list',
        items: ['AI 채팅 대화 기록 (질문·답변 내용)', '일일 토큰 사용량', '로그인 기록'],
      },
      { type: 'text', value: '위 항목 외의 개인정보는 수집하지 않습니다.' },
    ],
  },
  {
    id: 'retention',
    ...
    content: [
      { type: 'text', value: '개인정보는 수집 목적이 달성된 후 지체 없이 파기합니다.' },
      {
        type: 'list',
        items: [
  ...
  },
  {
    ...
    content: [
      {
        type: 'text',
        value: '서비스는 개인정보의 안전성 확보를 위해 다음과 같은 조치를 취하고 있습니다.',
      },
      {
        type: 'list',
        items: [
  ...
  },
  {
    ...
    content: [
      {
        type: 'text',
        value: '서비스는 서비스 제공을 위해 다음과 같이 개인정보 처리를 위탁하고 있습니다.',
      },
      {
        type: 'table',
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'table',
        headers: ['AI 서비스', '제공자', '전송 데이터', '식별정보 전송'],
        rows: [
      ...
      { type: 'subtitle', value: '예외적 제3자 제공' },
      {
        type: 'list',
        items: [
          '정보주체가 사전에 동의한 경우',
  ...
  },
  {
    ...
    content: [
      {
        type: 'text',
        value: '사용자(정보주체)는 다음과 같은 권리를 행사할 수 있습니다.',
      },
      {
        type: 'list',
        items: [
      ...
      { type: 'subtitle', value: '권리 행사 방법' },
      {
        type: 'text',
        value:
          '위 권리는 개인정보 보호책임자에게 이메일(greatsong21@gmail.com)로 요청하실 수 있습니다. 요청 접수 후 10일 이내에 처리 결과를 안내드립니다.',
      },
      { type: 'subtitle', value: '학생 및 법정대리인의 권리' },
      {
        type: 'list',
        items: [
  ...
  },
  {
    ...
    content: [
      {
        type: 'text',
        value: '서비스의 개인정보 보호책임자는 다음과 같습니다.',
      },
      {
        type: 'list',
        items: [
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'list',
        items: ['공고일자: 2026년 3월 18일', '시행일자: 2026년 3월 18일'],
      },
    ],
  },
];
    ...
    <div className="space-y-3">
      {blocks.map((block, i) => {
        switch (block.type) {
          case 'text':
            return (
              <p key={i} className="text-sm leading-relaxed text-gray-600 whitespace-pre-line">
                {block.value}
              </p>
            );
          case 'subtitle':
            return (
              <h3 key={i} className="text-sm font-semibold text-gray-800 mt-2">
                {block.value}
              </h3>
            );
          case 'list':
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {block.rows.map((row, j) => (
                      <tr key={j} className={j % 2 === 1 ? 'bg-gray-50/50' : ''}>
                        {row.map((cell, k) => (
                          <td
                            key={k}
        ...
        <div className="rounded-2xl shadow-lg border border-gray-200 p-8 mb-6 bg-white">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2 text-xs">
              <button onClick={expandAll} className="text-indigo-600 hover:underline">
                모두 펼치기
              </button>
              <span className="text-gray-300">|</span>
              <button onClick={collapseAll} className="text-indigo-600 hover:underline">
                모두 접기
              </button>
            </div>
          </div>
          ...
          <p className="text-sm leading-relaxed text-gray-500">
            당곡고 학생을 위한 AI 채팅(이하 &lsquo;서비스&rsquo;)은 「개인정보 보호법」 제30조에
            따라 정보주체의 개인정보를 보호하고 이와 관련한 고충을 신속하고 원활하게 처리할 수
